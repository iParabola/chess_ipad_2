	...
	<view>
		<uni-popup ref="popup" @maskClick="maskClick">
			<view class="rule__view">
				<view class="title">反装甲火力打击效果表</view>
				<uni-table border>
					...
					<uni-tr class="row" v-for="(item, index) in dataArray" :key="index">
						<uni-td align="center" :id="'y_' + item.diceScore">
							{{ item.diceScore }}
						</uni-td>
						<uni-td align="center" v-for="(eitem, eindex) in item.blowEffects.split(',')" :key="eindex">
							{{ eitem }}
						</uni-td>
					</uni-tr>
				</uni-table>
...
export default {
	...
	methods: {
		...
		sign() {
			this.$nextTick(() => {
				setTimeout(function () {
					const element = document.getElementById('x_8'); // 获取指定元素
					...
					const element2 = document.getElementById('y_3'); // 获取指定元素
