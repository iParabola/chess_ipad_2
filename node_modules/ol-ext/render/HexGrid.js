...
var ol_HexGrid = class olHexGrid extends ol_Object {
	constructor(options) {
		...
		this.col_ = options.col || 10;
		this.row_ = options.row || 10;
		this.coordinateMap = options.coordinateMap || {};
		this.imageMap = options.imageMap || {};

		console.log(this.col_)
	...
	hex_corner(center, size, i) {
		return [center[0] + size * this.layout_[8 + (2 * (i % 6))], center[1] + size * this.layout_[9 + (2 * (
			i % 6))]];
	}
	...
	cube_neighbors(c, d) {
		if (d !== undefined) {
			return [c[0] + this.neighbors.cube[d % 6][0], c[1] + this.neighbors.cube[d % 6][1], c[2] + this
				.neighbors.cube[d % 6][2]
			];
		} else {
			var n = [];
			for (d = 0; d < 6; d++) {
				n.push([c[0] + this.neighbors.cube[d][0], c[1] + this.neighbors.cube[d][1], c[2] + this
					.neighbors.cube[d][2]
				]);
			}
			for (d = 0; d < 6; d++)
